...
Serves real-time rover data via REST API and WebSocket
"""
import asyncio
import json
import threading
...
<body>
    <div class="container">
        <div class="header">
            <h1>🤖 PiRover Telemetry Dashboard</h1>
...
def parse_rover_data(data_string):
    """Parse PiRover data string format: D{distance}S{speed}M{mode}I{ir_bits}"""
    try:
...
def ble_scanner_thread():
    ...
    def callback(device: BLEDevice, adv: AdvertisementData):
        ...
        if parsed:
            ...
            # Print to console
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] PiRover Update")
...
def main():
    ...
    print("="*60)
    print("🤖 PiRover BLE Scanner with Flask Server")
    print("="*60)
